...
import cv2
import numpy as np
import seguimiento_manos as sm  #Programa que contiene la deteccion y seguimiento de manos
...
detector = sm.DetectorManos(max_manos=1) #Ya que solo vamos a utilizar una mano

while True:
    #----------------- Vamos a encontrar los puntos de la mano -----------------------------
    ret, frame = cap.read()
    frame = detector.encontrar_manos(frame)  #Encontramos las manos
    lista, bbox = detector.encontrar_posicion(frame) #Mostramos las posiciones
    ...
    if len(lista) != 0:
        ...
        dedos = detector.dedos_arriba() #Contamos con 5 posiciones nos indica si levanta cualquier dedo
        #print(dedos)
